...
int has_cycle(node *head) {
        ...
        // create node to skip by one, and node to skip by two
        struct node *oneNode = head->next;
        struct node *twoNode = (head->next)->next;

        // loop as long as nodes do not line up
        while (oneNode != twoNode && twoNode->next != oneNode) {

                // if any node or node this loop attempts to point to is null, no cycle
                if (((oneNode == NULL || twoNode == NULL)
                    || (oneNode->next == NULL || twoNode->next == NULL))
                    || (twoNode->next)->next == NULL) {
                        return 0;
                }
        ...
        // if nodes line up, there is a cycle
        return 1;
}
