...
// PillarBot Node creates a Navigator instance and controls it
// to patrol the map using map data already known. The patrol
// is on a timer repeat until a mismatched pillar is found
class PillarBot : public rclcpp::Node {
public:
	PillarBot():Node("finalprojectnav") {
		// Unused; meant to recieve a flag when pillar is found
		// to stop the bot's current goal. The while loop in 
		// the go function prevents this from working but yet 
		// to find a different method
		sub = this->create_subscription<std_msgs::msg::Bool>("/pillar", 10, std::bind(&PillarBot::pillarFound, this, std::placeholders::_1));
...
private:
	// create Navigator instance to control bot
	...
	// Unused
	// callback function that takes a flag indicated a pillar
	// has been found and to cancel patrolling
	void pillarFound(const std_msgs::msg::Bool::SharedPtr msg) {
		if (msg->data) found = true;
		RCLCPP_ERROR(this->get_logger(),"Found a pillar!");
		bot->CancelTask();
		return;
	}
	
	rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr sub; // Unused
};
...
class TFListener : public rclcpp::Node {
	public:
	TFListener() : Node("tflistener"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
		// Unused
		// created to post shutdown flag
		pub = this->create_publisher<std_msgs::msg::Bool>("/pillar", 10);
		
		// recieve map data to occupancy_grid
		subMap = this->create_subscription<nav_msgs::msg::OccupancyGrid>("/global_costmap/costmap",10,std::bind(&TFListener::checkMap,this,std::placeholders::_1));
		// recieve laser scan data for use in transforming to
		// new_map data (LaserScan->PointCould2->OccupancyGrid)
		subScan = this->create_subscription<sensor_msgs::msg::LaserScan>("/scan",10,std::bind(&TFListener::scan,this,std::placeholders::_1));
	}
	...
	std_msgs::msg::Bool found = std_msgs::msg::Bool(); // Unused
		...
		for (unsigned int i = 0; i < grid.info.height; ++i) {
			for (unsigned int j = 0; j < grid.info.width; ++j) {
				...
				if (grid.data[index] == 100 && first_map->data[index] < 0.65) {
					...
					if (num_mismatch > 5) {
						found.data = true; // Unused
						RCLCPP_ERROR(this->get_logger(),"Pillar found! (%d,%d)",i,j);
						shutdown();
						//pub->publish(found); // Unused
						break;
					}
				}
			}
			if (found.data) break; // Unused
		}
	...
	rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr pub; // Unused
...
int main(int argc,char **argv) {
  ...
  auto bot = std::make_shared<PillarBot>();
  auto listener = std::make_shared<TFListener>();
  
  // multithreaded to run both nodes
