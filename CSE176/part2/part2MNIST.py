...
# Use feature importances for a 28x28 heatmap recreation
grid = np.zeros((28, 28))
feature_indices = np.argsort(feature_importances)
for idx, importance in zip(feature_indices, feature_importances):
    row = idx // 28
    col = idx % 28
...
plt.savefig('test_error_vs_num_trees.eps', format='eps')
plt.show()
...
# Generate 28x28 image of feature importance
